...
*Contributor note — =diff-pdf= on PATH*

- *macOS (Homebrew)*: Batch Emacs inherits =PATH= from the environment that runs =make test=. If the wrong =diff-pdf= is used, set =RESUMEL_DIFF_PDF= to the full path of the binary you want.
- *Linux*: Install the distro package (e.g. Debian/Ubuntu: =apt install diff-pdf=) or ensure the binary is on =PATH=.

When syncing color or class definitions with upstream sources, local clones under =~/Repos/= are convenient (e.g. =awesomecv=, =altacv=, =moderncv=, =jakesresume=); those paths are not part of the repository.
