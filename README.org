...
- *Linux*: Install via [[https://brew.sh][Homebrew on Linux]] (=brew install diff-pdf=), build [[https://github.com/vslavik/diff-pdf][from source]] following the upstream README, or use your distribution’s packages where they exist (e.g. Fedora/CentOS: =dnf install diff-pdf= per upstream).
